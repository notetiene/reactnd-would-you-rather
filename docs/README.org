 ...
 * =loading= that is a =Boolean=
 * =router= that is a complex object
 * =redirect= that is a simple object
...
**** =loading=
#+begin_src plantuml :file images/loading_structure.png
  skinparam {
      titleBorderRoundCorner 15
      titleBorderThickness 2
      titleBorderColor black
  ...
  title =Structure of <u>loading</u> object
...
#+RESULTS:
[[file:images/loading_structure.png]]

**** =router=
This property is managed by =connected-react-router=.

**** =redirect=
#+begin_src plantuml :file images/redirect_structure.png
  skinparam {
      titleBorderRoundCorner 15
      titleBorderThickness 2
      titleBorderColor black
  ...
  title =Structure of <u>redirect</u> object

  left to right direction

  package redirect [{
      <i>lastLocation</i>: "/leaderboard",
  }]
#+end_src

#+caption: Questions structure
#+RESULTS:
[[file:images/redirect_structure.png]]

The redirect property is a simple object containing only one inner
property:
 * =lastLocation= that is used to record the location before a
   redirection
...
must use a component that will provide that behaviour.
...
#+begin_src plantuml :file images/signin_component_data.png
  ...
  title =Data of component: ""SignIn""
  left to right direction
...
*** =About= Component
The =About= component will display information about the application.
...
#+begin_src js :tangle ../src/reducers/index.js :comments link
...
#+begin_src js :tangle ../src/middleware/index.js :comments link
...
*** =configureStore.js=
#+begin_src js :tangle ../src/configureStore.js :comments link
...
#+begin_src js :tangle ../src/actions/authedUser.js :comments link
...
#+begin_src js :tangle ../src/actions/users.js :comments link
...
#+begin_src js :tangle ../src/actions/questions.js :comments link
import {
  saveQuestionAnswer,
  saveQuestion,
} from '../util/api';
...
*** loading
=loading= is a boolean that record if the application is doing API
calls or not.

#+begin_src js :tangle ../src/actions/loading.js :comments link
...
*** redirect
=redirect= will contain the previous location before redirection.

The =replace= function from =connected-react-router= will be used
instead of =push= so that the redirection is not pushed to the
history.

#+begin_src js :tangle ../src/actions/redirect.js :comments link
...
A thunk action creator will redirect to SignIn after having recorded
...
A thunk action creator will undo the redirection.  It simply means it
will /redirect/ to the recorded location and notify the reducer to
remove it.
...
#+begin_src js :tangle ../src/actions/shared.js :comments link
...
** Reducers
*** authedUser
#+begin_src js :tangle ../src/reducers/authedUser.js :comments link
import {
  SET_AUTHED_USER,
  SIGNOUT_AUTHED_USER,
...
*** users
#+begin_src js :tangle ../src/reducers/users.js :comments link
...
When receiving users, simply spread them.
#+begin_src js :tangle ../src/reducers/users.js :comments link
    case RECEIVE_USERS:
      return {
        ...state,
...
When answering a question, add the answer to the user object.  That’s
redundant, but that’s the way the provided =_DATA.js= file is
structured.
...
*** questions
#+begin_src js :tangle ../src/reducers/questions.js :comments link
import {
  RECEIVE_QUESTIONS,
  ANSWER_QUESTION,
  ADD_NEW_QUESTION,
...
When receiving questions, simply spread them.
#+begin_src js :tangle ../src/reducers/questions.js :comments link
...
When answering a question, only add the new user its votes property.
#+begin_src js :tangle ../src/reducers/questions.js :comments link
    case ANSWER_QUESTION:
      return {
        ...state,
...
Adding a new question creates a whole new question object.
#+begin_src js :tangle ../src/reducers/questions.js :comments link
...
*** loading
#+begin_src js :tangle ../src/reducers/loading.js :comments link
import {
  LOADING_START,
  LOADING_END,
...
*** redirect
#+begin_src js :tangle ../src/reducers/redirect.js :comments link
...
When there’s a redirection, record the current location.
#+begin_src js :tangle ../src/reducers/redirect.js :comments link
